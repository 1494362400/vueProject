  ...
  <div id="app">
    <div class="allBox">
      <ul class="all">
        <li class="Box" v-for="(item, idx) in allData" :key="idx">
          ...
          <div class="content">
            <div class="searchBox">
              <input type="text" placeholder="输入搜索内容" v-model="item.searchText" />
            </div>
            <ul>
              <li v-for="(dataItem, index) in ev_search(item)" :key="index">
                <input
                  type="checkbox"
                  @click="ev_check(item, index)"
      ...
      <div class="btnBox">
        <input type="button" value=">" @click="ev_go(0)" />
        <input type="button" value="<" @click="ev_go(1)" />
      </div>
    </div>
    <page :num="num" class="pageBox" ref="pageComponent">
      <div slot="jump">
        ...
        页
        <input type="button" value="跳转" @click="ev_jump()" />
      </div>
    </page>
  </div>
</template>
  ...
  methods: {
    ev_search(data){
      ...
      for(var i=0;i<data.mess.length;i++){
        if(data.mess[i].text.includes(data.searchText)){
          dataArr.push(data.mess[i])
        }
      }
      // data.forEach(item=>{
      //   if(item.text.contains(this.searchText)){
      //     dataArr.push(item)
      //   }
      // })
      return dataArr
    ...
    ev_jump() {
      if (parseInt(this.jumpPage) && parseInt(this.jumpPage)<=this.num && parseInt(this.jumpPage)>0) {
        this.$refs.pageComponent.ev_jump(this.jumpPage);
      }
    },
    ev_check(item, index) {
